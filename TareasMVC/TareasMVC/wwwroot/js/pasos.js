...
async function manejarClickSalvarPaso(paso) {
    ...
    if (!descripcion) {
        ...
        if (esNuevo) tareaEditarVM.pasos.pop();
...
async function insertarPaso(paso, data, idTarea) {
    ...
    if (respuesta.ok) {
        ...
        if (paso.realizado()) tarea.pasosRealizados(tarea.pasosRealizados() + 1);
    } else {
...
function manejarClickDescripcion(paso) {
...
async function actualizarPaso(data, id) {
    ...
    if (!respuesta.ok) manejarErrorApi(respuesta);
}

function manejarClickCheckboxPaso(paso) {
    if (paso.esNuevo()) return true;
    ...
    if (paso.realizado()) pasosRealizadosActual++;
    else pasosRealizadosActual--;
